...
const replyTokenSchema = z
  .string()
  .describe(
    "The reply token received from a webhook event. Used to reply to a specific message.",
  );
...
server.tool(
  "reply_text_message",
  "Reply to a specific message with a simple text message via LINE. Use this for sending plain text messages without formatting in response to a user's message.",
  {
...
server.tool(
  "reply_flex_message",
  "Reply to a specific message with a highly customizable flex message via LINE. Supports both bubble (single container) and carousel (multiple swipeable bubbles) layouts.",
  {
    replyToken: replyTokenSchema,
    message: flexMessageSchema,
  },
...
async function main() {
  ...
  console.log("Starting LINE Bot MCP server...");
  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.log("LINE Bot MCP server connected");
}
